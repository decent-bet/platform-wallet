...
export async function fetchCurrentSessionId() {
    try {
        ...
        console.log('Error retrieving current session', e.message)
    }
}
...
async function fetchUserCreditsForSession(sessionNumber) {
    try {
        let defaultAccount = helper.getDevWeb3().eth.defaultAccount
        return helper
            .getContractHelper()
            .getWrappers()
            .house()
...
async function fetchSessionData() {
    try {
        ...
        console.log('Error retrieving session details', err.message)
    }
}
...
async function fetchLottery(session) {
    try {
        ...
        let address = helper.getDevWeb3().eth.defaultAccount
...
async function fetchHouseAllowance() {
    try {
        return helper
            ...
            .testDecentBetToken()
            .allowance(
                helper.getDevWeb3().eth.defaultAccount,
